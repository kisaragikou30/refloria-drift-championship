...
$('driverForm').onsubmit=async e=>{
  e.preventDefault();
  const number=$('driverNumber').value.trim();
  const name=$('driverName').value.trim();
  const car=$('driverCar').value.trim();
  const team=$('driverTeam').value.trim();
  const photo_url=$('driverPhoto').value.trim();
  if(!number||!name){setText('driverMsg','番号と選手名を入力してください。',true);return;}
  const btn=e.target.querySelector('button[type=submit]');
  btn.disabled=true;
  setText('driverMsg','登録中...');
  const {error}=await client.from('drivers').insert({number,name,car,team,photo_url});
  btn.disabled=false;
  if(error){setText('driverMsg','登録失敗: '+error.message,true);return;}
  e.target.reset();
  setText('driverMsg','選手を追加しました ✓');
  await loadDrivers();
...
async function loadDrivers(){
  const el=$("driverList");
  if(!el||!client)return;
  const {data,error}=await client.from("drivers").select("id,number,name,car,team,photo_url").order("number",{ascending:true});
  if(error){el.innerHTML=`<p style="color:#ff7d8c">${esc(error.message)}</p>`;return;}

  // 現在選択中の大会ポイントも一緒に読み込む
  const eventId=Number($("pointEvent")?.value||0);
  let pointMap={};
  if(eventId){
    const pr=await client.from("driver_event_points").select("driver_id,points").eq("event_id",eventId);
    if(!pr.error)(pr.data||[]).forEach(p=>pointMap[p.driver_id]=Number(p.points||0));
  }
  ...
  el.innerHTML=(data||[]).map(d=>`
    <div class="driver">
      ...
      <span>${d.photo_url?'<span class="photo-ok">登録済み</span>':'—'}</span>
      <span class="driver-point-cell">
        ${eventId
          ? `<input class="driver-point-input" type="number" min="0" step="1" value="${Number(pointMap[d.id]||0)}" data-driver-point="${Number(d.id)}" aria-label="${esc(d.name||"選手")}のポイント">`
          : '<span>大会を選択</span>'}
      </span>
      <span class="driver-actions">
        <button type="button" class="photo-edit" onclick="editDriverPhoto(${Number(d.id)})">写真</button>
        <button type="button" onclick="deleteDriver(${Number(d.id)})">削除</button>
      </span>
    </div>`).join("")||"<p>選手がまだ登録されていません。</p>";
}
...
$("savePointsBtn").onclick=async()=>{
  const eventId=Number($("pointEvent").value);
  if(!eventId)return setText("pointMsg","大会を選択してください。",true);
  const inputs=[...document.querySelectorAll("[data-driver-point]")];
  if(!inputs.length)return setText("pointMsg","選手がいません。",true);
  const rows=inputs.map(i=>({event_id:eventId,driver_id:Number(i.dataset.driverPoint),points:Math.max(0,Number(i.value)||0),updated_at:new Date().toISOString()}));
  const {error}=await client.from("driver_event_points").upsert(rows,{onConflict:"event_id,driver_id"});
  if(error){setText("pointMsg",error.message,true);return;}
  setText("pointMsg","各選手のポイントを保存しました ✓");
  await loadPointEditor();
  await loadDrivers();
};

$("pointEvent").addEventListener("change",()=>loadDrivers());
